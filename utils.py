...
def _kmeans_iter(X, x_sq_norms, centres, update_centres=True):
...
def _kmeans_single(X, x_sq_norms, centres, max_iter=300, tol=1e-4):
    labels_old = None
    # labels.clone()
    strict_convergence = False

    for i in range(max_iter):
        centres_new, weight_in_clusters, labels, centre_shift = _kmeans_iter(
            X,
            x_sq_norms,
            centres
        )
...
class KMeans:
    ...
    tol : float, default=1e-4
        ...
        convergence.
    """
    def __init__(self, n_clusters, n_init=10, max_iter=300, tol=1e-4):
    ...
    def fit(self, X):
        ...
        for i in range(self.n_init):
            centres = _init_centroids(X, self.n_clusters, x_sq_norms)
            labels, inertia, centres, n_iter_ = _kmeans_single(
                X,
                x_sq_norms,
                centres,
